...
using REAL salon assets:
  • videos/before-after.mp4  (short before→after transition, poster below)
...
function styles() {
  ...
  st.textContent =
    ...
    ".nl-ba--three{grid-template-columns:repeat(3,1fr);max-width:760px;margin-top:16px}" +
    "@media(max-width:560px){.nl-ba--three{grid-template-columns:1fr}}";
...
function inject() {
  ...
  styles();
  var imgs = RESULTS.map(function (src, i) {
  ...
  sec.innerHTML =
    '<div class="container">' +
      ...
      '</div>' +
      '<div class="nl-ba nl-ba--three reveal">' + imgs + '</div>' +
